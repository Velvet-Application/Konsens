...
export default function AcademyV2(){
  ...
  const completed=new Set(progress.filter(p=>p.completed_at).map(p=>p.module_id));
  const minutes=courses.reduce((n,c)=>n+c.duration_minutes,0);
  ...
  return <section className="academy-v2">
    ...
    {selected&&<CourseReader course={selected} progress={progress.find(p=>p.module_id===selected.id)} close={()=>setSelected(null)} completed={(score)=>setProgress(old=>[...old.filter(p=>p.module_id!==selected.id),{module_id:selected.id,completed_at:new Date().toISOString(),score}])}/>} 
  </section>;
}

function CourseReader({course,progress,close,completed}:{course:Course;progress?:Progress;close:()=>void;completed:(score:number)=>void}){
  const[chapter,setChapter]=useState(0);
  const[answers,setAnswers]=useState<Record<number,number>>({});
  const[result,setResult]=useState<number|null>(progress?.score??null);
  const[status,setStatus]=useState("");
  const current=course.content_json[chapter];
  const answered=Object.keys(answers).length===course.quiz_json.length;
  const score=useMemo(()=>course.quiz_json.length?Math.round(course.quiz_json.reduce((n,q,i)=>n+(answers[i]===q.answer?1:0),0)/course.quiz_json.length*100):100,[answers,course.quiz_json]);

  const validate=async()=>{
    if(!answered&&course.quiz_json.length)return;
    const {data:auth}=await supabase.auth.getUser();
    if(!auth.user){setStatus("Reconnecte-toi pour enregistrer ta progression.");return}
    const finalScore=course.quiz_json.length?score:100;
    setStatus("Enregistrement de ta progression…");
    const {error}=await supabase.from("learning_progress").upsert({user_id:auth.user.id,module_id:course.id,completed_at:new Date().toISOString(),score:finalScore},{onConflict:"user_id,module_id"});
    if(error){setStatus(`Impossible d’enregistrer : ${error.message}`);return}
    setResult(finalScore);
    setStatus(finalScore>=70?"Progression enregistrée.":"Score enregistré. Reprends les chapitres puis retente le quiz.");
    if(finalScore>=70)completed(finalScore);
  };
