        ...
        ScrollView {
            LazyVStack(alignment: .leading, spacing: 14) {
                ...
                if store.markets.isEmpty {
                    VStack(spacing: 12) {
                        Image(systemName: "checkmark.shield.fill").font(.largeTitle).foregroundStyle(.konsensGreen)
                        ...
                        Text("Les prédictions ne sont publiées qu’après validation de la question, de la source et de la règle de résolution.")
                            .font(.caption).foregroundStyle(.konsensMuted).multilineTextAlignment(.center)
...
private struct MarketCard: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 14) {
            HStack { Eyebrow(text: market.category.uppercased()); Spacer(); Text("\(market.yesProbability)% OUI").font(.caption.monospacedDigit().bold()).foregroundStyle(.konsensGreen) }
            ...
            HStack { Text("Clôture \(shortDate(market.closesAt))").font(.caption2).foregroundStyle(.konsensMuted); Spacer(); Text("Mise \(amount) Koins").font(.caption2).foregroundStyle(.konsensMuted) }
            HStack(spacing: 9) {
                Button { Task { await store.bet(market, outcome: "yes", amount: amount) } } label: { Text("OUI · \(amount)").frame(maxWidth: .infinity).padding(13).background(Color.konsensPositive.opacity(0.12), in: RoundedRectangle(cornerRadius: 13)).overlay(RoundedRectangle(cornerRadius: 13).stroke(Color.konsensPositive.opacity(0.22))) }.foregroundStyle(.konsensPositive)
...
struct AmountPicker: View {
    ...
    var body: some View {
        HStack(spacing: 7) {
            ...
            Spacer(); Text("Koins").font(.caption2).foregroundStyle(.konsensMuted)
...
struct RiskFooter: View {
    var body: some View {
        HStack(alignment: .top, spacing: 10) {
            Image(systemName: "exclamationmark.shield.fill").foregroundStyle(.konsensGold)
            Text("Risque visible, argent fictif. Une forte performance virtuelle peut être suivie d’une perte importante. Les Koins ne sont ni achetables, ni convertibles, ni retirable en argent.")
                .font(.system(size: 9)).foregroundStyle(.konsensMuted)
