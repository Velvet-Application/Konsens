    ...
    var body: some View {
        HStack(spacing: 13) {
            Text(String(asset.symbol.prefix(2)))
                .font(.caption.black()).foregroundStyle(.konsensGreen)
                .frame(width: 46, height: 46).background(Color.konsensGreen.opacity(0.1), in: RoundedRectangle(cornerRadius: 14))
            VStack(alignment: .leading, spacing: 4) {
                Text("\(asset.kind.uppercased()) · SIMULATION").font(.system(size: 7, weight: .bold)).foregroundStyle(.konsensMuted)
                Text(asset.name).font(.headline)
                Text("Valeur école \(asset.price, specifier: "%.2f")").font(.caption2).foregroundStyle(.konsensMuted)
            }
            Spacer()
...
private struct PremiumMarketsCard: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 10) {
            HStack { Image(systemName: "crown.fill").foregroundStyle(.konsensViolet); Eyebrow(text: "PREMIUM · 4,99 € / MOIS") }
            ...
            Label(store.subscriptionTier == "premium" ? "Premium actif" : "Bientôt disponible", systemImage: store.subscriptionTier == "premium" ? "checkmark.seal.fill" : "lock.fill")
                .font(.caption.bold()).foregroundStyle(.konsensViolet)
